...
export function AddAccountDialog({ isOpen, onClose }: AddAccountDialogProps): React.ReactNode {
  ...
  useEffect(() => {
    ...
    const unsubscribe = window.api.onSocialAuthCallback(async (data) => {
      ...
      if (data.error) {
        setError(`${t('add_account_dialog.login_failed')}: ${data.error}`)
      ...
      if (data.code && data.state) {
        try {
          ...
          if (result.success) {
            ...
            setError(result.error || t('add_account_dialog.token_exchange_failed'))
          }
        } catch (e) {
          setError(e instanceof Error ? e.message : t('add_account_dialog.login_failed'))
        } finally {
          setIsLoggingIn(false)
        }
      }
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        if (isAccountExists(email, userId)) {
          setError(t('add_account_dialog.account_exists'))
          return
        }
        ...
        setError(result.error || t('add_account_dialog.verify_failed'))
      }
    } catch (e) {
      setError(e instanceof Error ? e.message : t('add_account_dialog.add_failed'))
    }
  }
  ...
  const handleStartBuilderIdLogin = async () => {
    ...
    try {
      ...
      if (result.success && result.userCode && result.verificationUri) {
        ...
        setError(result.error || t('add_account_dialog.start_login_failed'))
        setIsLoggingIn(false)
      }
    } catch (e) {
      setError(e instanceof Error ? e.message : t('add_account_dialog.start_login_failed'))
      setIsLoggingIn(false)
    }
  }
  ...
  const startPolling = (interval: number) => {
    ...
    pollIntervalRef.current = setInterval(async () => {
      try {
        ...
        if (!result.success) {
          setError(result.error || t('add_account_dialog.auth_failed'))
          setIsLoggingIn(false)
          setBuilderIdLoginData(null)
          if (pollIntervalRef.current) {
            clearInterval(pollIntervalRef.current)
  ...
  const handleImportFromLocal = async () => {
    try {
      ...
      if (result.success && result.data) {
        ...
        setError(result.error || t('add_account_dialog.import_failed'))
      }
    } catch (e) {
      setError(e instanceof Error ? e.message : t('add_account_dialog.import_failed'))
    }
  }
  ...
  const handleSsoImport = async () => {
    if (!ssoToken.trim()) {
      setError(t('add_account_dialog.enter_sso_token'))
      return
    }
    ...
    if (tokens.length === 0) {
      setError(t('add_account_dialog.enter_at_least_one_token'))
      return
    }
      ...
      try {
        ...
        if (result.success && result.data) {
          ...
          if (email && userId && isAccountExists(email, userId)) {
            importResult.errors.push(`#${index + 1}: ${email} ${t('add_account_dialog.already_exists')}`)
            return
          }
          ...
          importResult.errors.push(`#${index + 1}: ${result.error?.message || t('add_account_dialog.import_failed')}`)
        }
      } catch (e) {
        importResult.failed++
        importResult.failedIndices.push(index)
        importResult.errors.push(`#${index + 1}: ${e instanceof Error ? e.message : t('add_account_dialog.import_failed')}`)
      }
    }
    ...
    try {
      ...
      if (importResult.failed === 0) {
        ...
        if (importResult.success > 0) {
          setError(t('add_account_dialog.import_result', { success: importResult.success, failed: importResult.failed }))
        } else {
          setError(t('add_account_dialog.import_all_failed', { failed: importResult.failed }))
        }
      }
    } catch (e) {
      setError(e instanceof Error ? e.message : t('add_account_dialog.sso_import_failed'))
    } finally {
  ...
  const handleOidcBatchAdd = async () => {
    if (!oidcBatchData.trim()) {
      setError(t('add_account_dialog.enter_credentials_data'))
      return
    }
    ...
    try {
      ...
      setError(t('add_account_dialog.invalid_json'))
      return
    }

    if (credentials.length === 0) {
      setError(t('add_account_dialog.enter_at_least_one_credential'))
      return
    }
    ...
    const importSingleCredential = async (cred: typeof credentials[0], index: number): Promise<void> => {
      try {
        if (!cred.refreshToken) {
          importResult.failed++
          importResult.failedIndices.push(index)
          importResult.errors.push(`#${index + 1}: ${t('add_account_dialog.missing_refresh_token')}`)
          return
        }
        ...
        if (result.success && result.data) {
          ...
          const errorMsg = typeof err === 'object' ? (err?.message || t('add_account_dialog.verify_failed')) : (err || t('add_account_dialog.verify_failed'))
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : t('add_account_dialog.oidc_batch_import_failed'))
    } finally {
  ...
  const handleOidcAdd = async () => {
    if (!refreshToken) {
      setError(t('add_account_dialog.fill_refresh_token'))
      return
    }
    // 社交登录不需要 clientId 和 clientSecret
    if (authMethod !== 'social' && (!clientId || !clientSecret)) {
      setError(t('add_account_dialog.fill_client_credentials'))
      return
    }
      ...
      <Card className="relative w-full max-w-lg max-h-[90vh] overflow-auto z-10">
        <CardHeader className="pb-4 border-b">
          <div className="flex flex-row items-center justify-between">
            <CardTitle className="text-xl font-bold">{t('accounts.add_account')}</CardTitle>
            <Button variant="ghost" size="icon" className="h-8 w-8 rounded-full hover:bg-muted" onClick={onClose}>
              <X className="h-4 w-4" />
            </Button>
          </div>
          <p className="text-sm text-muted-foreground mt-1">{t('add_account_dialog.subtitle')}</p>
        </CardHeader>
        ...
        <CardContent className="space-y-6 pt-6">
          ...
          <div className="grid grid-cols-3 gap-1 p-1 bg-muted/50 rounded-xl border">
            ...
            >
              {t('add_account_dialog.online_login')}
            </button>
            <button
              className={`py-2 px-3 text-sm rounded-lg transition-all duration-200 font-medium ${
                importMode === 'oidc' 
              ...
              disabled={!!verifiedData || isLoggingIn}
            >
              {t('add_account_dialog.oidc_credentials')}
            </button>
            <button
              className={`py-2 px-3 text-sm rounded-lg transition-all duration-200 font-medium ${
                importMode === 'sso' 
              ...
              disabled={!!verifiedData || isLoggingIn}
            >
              {t('add_account_dialog.sso_token')}
            </button>
          </div>
                ...
                <div className="space-y-4">
                  <div className="p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg text-center">
                    <p className="text-sm text-blue-700 dark:text-blue-300 mb-2">
                      {t('add_account_dialog.complete_login_in_browser')}
                    </p>
                    ...
                    <div className="mt-3 flex items-center justify-center gap-2 text-xs text-muted-foreground">
                      ...
                      {t('add_account_dialog.waiting_authorization')}
                    </div>
                  </div>
                  ...
                  <div className="flex gap-2">
                    ...
                    >
                      ...
                      {t('add_account_dialog.reopen_browser')}
                    </Button>
                    <Button 
                      variant="destructive" 
                      className="flex-1"
                    ...
                    >
                      {t('add_account_dialog.cancel_login')}
                    </Button>
                  </div>
                </div>
              )}
                ...
                <div className="space-y-4">
                  <div className="p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg text-center">
                    ...
                    <p className="text-sm text-blue-700 dark:text-blue-300">
                      {t('add_account_dialog.complete_login_in_browser_short')}
                    </p>
                    <p className="text-xs text-muted-foreground mt-1">
                      {t('add_account_dialog.auto_return_after_login')}
                    </p>
                  </div>
              ...
              {!isLoggingIn && (
                <div className="space-y-4 py-2">
                  <div className="text-center mb-6">
                    ...
                    <h3 className="text-lg font-semibold">{t('add_account_dialog.choose_login_method')}</h3>
                    <p className="text-sm text-muted-foreground mt-1">
                      {t('add_account_dialog.quick_login_supported')}
                    </p>
                  </div>
                  ...
                  <div className="space-y-3 px-2">
                    ...
                    >
                      ...
                      <div className="flex flex-col items-start">
                        <span className="text-sm font-semibold text-foreground">{t('add_account_dialog.google_account')}</span>
                        <span className="text-xs text-muted-foreground">{t('add_account_dialog.google_login_desc')}</span>
                      </div>
                    </button>
                    ...
                    >
                      ...
                      <div className="flex flex-col items-start">
                        <span className="text-sm font-semibold text-foreground">{t('add_account_dialog.github_account')}</span>
                        <span className="text-xs text-muted-foreground">{t('add_account_dialog.github_login_desc')}</span>
                      </div>
                    </button>
                    ...
                    >
                      ...
                      <div className="flex flex-col items-start">
                        ...
                        <span className="text-xs text-muted-foreground">{t('add_account_dialog.builderid_login_desc')}</span>
                      </div>
                    </button>
                  </div>
                </div>
              ...
              <div className="p-4 bg-blue-50/50 dark:bg-blue-900/10 rounded-xl border border-blue-100 dark:border-blue-900/20">
                <div className="flex items-start gap-3">
                   ...
                   <div className="flex-1">
                      <p className="text-sm font-semibold text-blue-700 dark:text-blue-300 mb-1.5">{t('add_account_dialog.how_to_get_token')}</p>
                      <ol className="text-xs text-blue-600/90 dark:text-blue-400/90 list-decimal list-inside space-y-1.5">
                        <li>{t('add_account_dialog.token_step_1')} <a href="https://view.awsapps.com/start/#/device?user_code=PQCF-FCCN/start/#/device?user_code=PQCF-FCCN" target="_blank" className="underline hover:text-blue-800 font-medium">view.awsapps.com/start/#/device?user_code=PQCF-FCCN</a></li>
                        <li>{t('add_account_dialog.token_step_2')}</li>
                        <li>{t('add_account_dialog.token_step_3')} <code className="px-1 py-0.5 bg-blue-100 dark:bg-blue-900/40 rounded font-mono text-[10px]">x-amz-sso_authn</code> {t('add_account_dialog.token_step_3_suffix')}</li>
                      </ol>
                   </div>
                </div>
              </div>
              ...
              <div className="space-y-4">
                <div className="space-y-2">
                  <label className="text-sm font-medium flex items-center gap-1">
                    ...
                    <span className="text-xs text-muted-foreground font-normal ml-2">{t('add_account_dialog.batch_import_each_line')}</span>
                  </label>
                  <textarea
                    ...
                    placeholder={t('add_account_dialog.token_placeholder')}
                    value={ssoToken}
                    onChange={(e) => { setSsoToken(e.target.value); setBatchImportResult(null) }}
                  />
                  {ssoToken.trim() && (
                    <p className="text-xs text-muted-foreground">
                      {t('add_account_dialog.tokens_entered', { count: ssoToken.split('\n').filter(t => t.trim()).length })}
                    </p>
                  )}
                </div>
              ...
              {batchImportResult && (
                <div className={`p-3 rounded-lg text-sm ${batchImportResult.failed > 0 ? 'bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800' : 'bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800'}`}>
                  <p className={`font-medium ${batchImportResult.failed > 0 ? 'text-amber-700 dark:text-amber-300' : 'text-green-700 dark:text-green-300'}`}>
                    {t('add_account_dialog.import_result_ratio', { success: batchImportResult.success, total: batchImportResult.total })}
                  </p>
              ...
              >
                {isVerifying ? (
                  <>
                    ...
                    {t('add_account_dialog.concurrent_importing_count', { count: ssoToken.split('\n').filter(t => t.trim()).length })}
                  </>
                ) : (
                  ssoToken.split('\n').filter(t => t.trim()).length > 1 
                    ? t('add_account_dialog.batch_import_count', { count: ssoToken.split('\n').filter(t => t.trim()).length })
                    : t('add_account_dialog.import_and_verify')
                )}
              </Button>
            </div>
          )}
          ...
          {importMode === 'oidc' && !verifiedData && (
            <div className="space-y-5">
              <div className="flex items-center justify-between">
                <h3 className="text-sm font-semibold">{t('add_account_dialog.input_oidc_credentials')}</h3>
                <div className="flex items-center gap-2">
                  ...
                  {oidcImportMode === 'single' && (
                    ...
                    >
                      ...
                      {t('add_account_dialog.import_from_local')}
                    </Button>
                  )}
                </div>
              </div>
              ...
              {oidcImportMode === 'single' && (
                <>
                  <div className="space-y-4">
                    {/* 登录类型选择 */}
                    <div className="space-y-2">
                      <label className="text-sm font-medium">{t('add_account_dialog.login_type')}</label>
                      <div className="flex gap-2">
                        <button
                          type="button"
                      ...
                      {authMethod === 'social' && (
                        <p className="text-xs text-muted-foreground">
                          {t('add_account_dialog.social_no_client_required')}
                        </p>
                      )}
                    </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <textarea
                        ...
                        placeholder={t('add_account_dialog.refresh_token_placeholder')}
                        value={refreshToken}
                ...
                <>
                  <div className="p-3 bg-blue-50/50 dark:bg-blue-900/10 rounded-xl border border-blue-100 dark:border-blue-900/20">
                    <p className="text-xs text-blue-600 dark:text-blue-400">
                      {t('add_account_dialog.json_array_supported')} <code className="px-1 bg-blue-100 dark:bg-blue-900/40 rounded">refreshToken</code>。
                      {t('add_account_dialog.optional_fields')}: <code className="px-1 bg-blue-100 dark:bg-blue-900/40 rounded">provider</code> (BuilderId/Github/Google)、
                  ...
                  <div className="space-y-2">
                    <label className="text-sm font-medium flex items-center gap-2">
                      {t('add_account_dialog.json_credentials_data')} <span className="text-destructive">*</span>
                    </label>
                    <textarea
    ...
    "provider": "Google"
                    ...
                    {oidcBatchData.trim() && (() => {
                      try {
                        ...
                        return <p className="text-xs text-muted-foreground">{t('add_account_dialog.credentials_entered', { count })}</p>
                      } catch {
                        return <p className="text-xs text-destructive">{t('add_account_dialog.json_format_error')}</p>
                      }
                    })()}
                  </div>
                  ...
                  {oidcBatchImportResult && (
                    <div className={`p-3 rounded-lg text-sm ${oidcBatchImportResult.failed > 0 ? 'bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800' : 'bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800'}`}>
                      <p className={`font-medium ${oidcBatchImportResult.failed > 0 ? 'text-amber-700 dark:text-amber-300' : 'text-green-700 dark:text-green-300'}`}>
                        {t('add_account_dialog.import_result_ratio', { success: oidcBatchImportResult.success, total: oidcBatchImportResult.total })}
                      </p>
            ...
            <div className="flex justify-end gap-3 pt-4 border-t">
              ...
              {oidcImportMode === 'single' ? (
                ...
                >
                  ...
                  {t('add_account_dialog.confirm_add')}
                </Button>
              ) : (
                <Button 
                ...
                >
                  {isSubmitting ? (
                    <>
                      <Loader2 className="h-4 w-4 mr-2 animate-spin" />
                      {t('add_account_dialog.concurrent_importing')}
                    </>
                  ) : (
                    (() => {
                      try {
                        const parsed = JSON.parse(oidcBatchData.trim())
                        const count = Array.isArray(parsed) ? parsed.length : 1
                        return t('add_account_dialog.batch_import_count', { count })
                      } catch {
                        return t('add_account_dialog.batch_import')
                      }
                    })()
                  )}
                </Button>
