...
// StatusLabels moved to component to use i18n

// 格式化 Token 到期时间
function formatTokenExpiry(expiresAt: number, t: any): string {
  const now = Date.now()
  ...
  if (minutes < 60) {
    ...
    return remainingHours > 0 ? `${days} ${t('common.days')} ${remainingHours} ${t('common.hours')}` : `${days} ${t('common.days')}`
  }
}
...
export const AccountCard = memo(function AccountCard({
  ...
  const handleSwitch = async (): Promise<void> => {
    ...
    if (!credentials.refreshToken) {
      alert(t('accounts.incomplete_credentials'))
      return
    }
              ...
              <div className="flex items-center justify-between gap-2">
                 ...
                 <div className={cn(
                    ...
                    "text-muted-foreground bg-muted"
                     ...
                     {isUnauthorized ? t('common.banned') : (
                       account.status === 'active' ? t('common.active') :
                       account.status === 'expired' ? t('common.expired') :
                       account.status === 'error' ? t('common.error') :
                       account.status === 'refreshing' ? t('common.refreshing') :
                       t('common.unknown')
                     )}
                 </div>
              </div>
        ...
        <div className="flex items-center gap-2 flex-wrap">
         ...
         <div className="bg-muted/30 p-3 rounded-lg space-y-2 border border-border/50">
             <div className="flex justify-between items-end text-xs">
                 <span className="text-muted-foreground font-medium">{t('accounts.account_usage')}</span>
             ...
             <div className="text-[10px] text-muted-foreground flex flex-col leading-tight gap-0.5">
                 <div className="flex items-center gap-1">
                    ...
                    <span className={isExpiringSoon ? "text-amber-600 font-medium" : ""}>
                       {account.subscription.daysRemaining !== undefined ? `${t('common.remaining')} ${account.subscription.daysRemaining} ${t('common.days')}` : '-'}
                    </span>
                 </div>
                 <div className="flex items-center gap-1" title={account.credentials.expiresAt ? new Date(account.credentials.expiresAt).toLocaleString(i18n.language) : t('common.unknown')}>
            ...
            <div className="flex items-center gap-0.5">
                ...
                {!account.isActive && (
                  <Button
                    ...
                    title={hasValidCredentials() ? t('accounts.switch_to_account') : t('accounts.incomplete_credentials')}
                  >
                    <Power className="h-3.5 w-3.5" />
                  </Button>
                )}
                
                <Button size="icon" variant="ghost" className="h-7 w-7 text-muted-foreground hover:text-foreground" onClick={(e) => { e.stopPropagation(); handleRefresh() }} disabled={account.status === 'refreshing'} title={t('accounts.check_account_info')}>
                   <RefreshCw className={cn("h-3.5 w-3.5", account.status === 'refreshing' && "animate-spin")} />
                </Button>
                <Button size="icon" variant="ghost" className="h-7 w-7 text-muted-foreground hover:text-foreground" onClick={(e) => { e.stopPropagation(); handleRefreshToken() }} disabled={isRefreshingToken} title={t('accounts.refresh_token')}>
                   <KeyRound className={cn("h-3.5 w-3.5", isRefreshingToken && "animate-pulse")} />
                </Button>
                
                <Button size="icon" variant="ghost" className={cn("h-7 w-7 text-muted-foreground hover:text-foreground", copied && "text-green-500")} onClick={(e) => { e.stopPropagation(); handleCopyCredentials() }} title={t('accounts.copy_credentials')}>
                   {copied ? <Check className="h-3.5 w-3.5" /> : <Copy className="h-3.5 w-3.5" />}
                </Button>

                <Button size="icon" variant="ghost" className="h-7 w-7 text-muted-foreground hover:text-foreground" onClick={(e) => { e.stopPropagation(); onShowDetail() }} title={t('common.details')}>
                   <Info className="h-3.5 w-3.5" />
                </Button>
                
                <Button size="icon" variant="ghost" className="h-7 w-7 text-muted-foreground hover:text-foreground" onClick={(e) => { e.stopPropagation(); onEdit() }} title={t('common.edit')}>
                   <Edit className="h-3.5 w-3.5" />
                </Button>
                
                <Button size="icon" variant="ghost" className="h-7 w-7 text-muted-foreground hover:text-destructive transition-colors" onClick={(e) => { e.stopPropagation(); handleDelete() }} title={t('common.delete')}>
