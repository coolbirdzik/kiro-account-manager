  ...
  const handleImport = async (): Promise<void> => {
    ...
    try {
      ...
      if ('isMultiple' in fileData && fileData.isMultiple) {
        ...
        for (const file of fileData.files) {
          ...
          try {
            ...
            if (data.version && data.account && !data.accounts) {
              ...
              if (result.success) {
                successCount++
              } else if (result.error === t('account_manager.account_exists')) {
                skippedCount++
              } else {
                failedCount++
              }
            ...
            // 完整导出数据格式
              ...
              if (skippedInfo) {
                const match = skippedInfo.error.match(/\d+/)
                if (match) skippedCount += parseInt(match[0])
              }
            } else {
              failedCount++
            }
        ...
        let msg = t('account_manager.batch_import_done', { success: successCount })
        if (skippedCount > 0) msg += `, ${t('account_manager.skipped_existing', { count: skippedCount })}`
        if (failedCount > 0) msg += `, ${t('account_manager.failed_count', { count: failedCount })}`
        alert(msg)
        return
      }
      ...
      if (format === 'json') {
        ...
        if (data.version && data.account && !data.accounts) {
          const result = importSingleAccount(data)
          if (result.success) {
            alert(t('account_manager.import_success_one'))
          } else {
            alert(`${t('account_manager.import_failed')}: ${result.error}`)
          }
          return
        }
        ...
        if (data.version && data.accounts) {
          ...
          alert(`${t('account_manager.import_done', { success: result.success })}${skippedMsg}`)
        } else {
          alert(t('account_manager.invalid_json_format'))
        }
      } else if (format === 'csv') {
        // CSV 格式：邮箱,昵称,登录方式,RefreshToken,ClientId,ClientSecret,Region
        const lines = content.split('\n').filter(line => line.trim())
        if (lines.length < 2) {
          alert(t('account_manager.empty_csv'))
          return
        }
        ...
        if (items.length === 0) {
          alert(t('account_manager.no_valid_data_need_email_token'))
          return
        }

        const result = importAccounts(items)
        alert(t('account_manager.import_done_with_failed', { success: result.success, failed: result.failed }))
      } else if (format === 'txt') {
        ...
        if (items.length === 0) {
          alert(t('account_manager.no_valid_data_format'))
          return
        }
        ...
        alert(`${t('account_manager.unsupported_file_format')}: ${format}`)
      }
    } catch (e) {
      console.error('Import error:', e)
      alert(t('account_manager.parse_import_failed'))
    }
  }
      ...
      {kiroDetected === false && (
        <div className="px-6 py-3 bg-amber-500/10 border-b border-amber-500/20 flex items-center justify-between">
          <div className="flex items-center gap-2 text-amber-600 dark:text-amber-400">
            ...
            <span className="text-sm">{t('machine_id.machine_id_info')}</span>
          </div>
      ...
      <header className="flex items-center justify-between gap-4 px-6 py-4 border-b bg-gradient-to-r from-primary/5 to-transparent">
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-lg font-semibold text-primary">{t('accounts.account_management')}</h1>
          </div>
          {/* Kiro 路径状态指示 */}
          {kiroDetected && kiroPath && (
            <Button 
              ...
              title={t('account_manager.kiro_path', { path: kiroPath })}
            >
              <Settings className="h-3 w-3" />
              {t('account_manager.kiro_configured')}
            </Button>
          )}
        </div>
