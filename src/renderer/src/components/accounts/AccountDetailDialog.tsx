...
// 格式化完整日期时间
const formatDateTime = (date: unknown, locale: string): string => {
  if (!date) return '-'
  try {
            ...
            <div>
              <div className="flex items-center gap-2">
               ...
               <div className="flex items-center gap-2 mt-1 text-xs text-muted-foreground">
                  ...
                  <span>·</span>
                  <span>{t('accounts.account_created')} {formatDate(account.createdAt)}</span>
               </div>
            </div>
          </div>
        ...
        <div className="p-6 space-y-8">
               ...
               <div>
                 <div className="flex items-end justify-between mb-2">
                   <div className="space-y-1">
                      <div className="text-sm text-muted-foreground font-medium">{t('home.total_quota')}</div>
                   ...
                   <div className={cn(
                     ...
                     "bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400"
                      ...
                      {(usage.percentUsed * 100).toFixed(1)}% {t('home.used')}
                   </div>
                 </div>
               ...
               <div className="grid grid-cols-3 gap-4 pt-2">
                 ...
                 <div className="p-4 bg-background rounded-xl border shadow-sm">
                   <div className="flex items-center gap-2 text-xs font-semibold text-blue-600 dark:text-blue-400 mb-2">
                    ...
                    <div className="text-xs text-muted-foreground mt-1 font-medium">
                      {formatDate(usage.nextResetDate)} {t('common.reset')}
                    </div>
                 </div>
                 ...
                 <div className={cn("p-4 bg-background rounded-xl border shadow-sm", (usage.freeTrialLimit ?? 0) === 0 && "opacity-60 grayscale")}>
                   <div className="flex items-center gap-2 text-xs font-semibold text-purple-600 dark:text-purple-400 mb-2">
                    ...
                    <div className="text-xs text-muted-foreground mt-1 font-medium">
                      {usage.freeTrialExpiry ? `${formatDate(usage.freeTrialExpiry)} ${t('common.expired')}` : t('accounts.no_accounts')}
                   </div>
                 </div>
                 ...
                 <div className={cn("p-4 bg-background rounded-xl border shadow-sm", bonusTotal === 0 && "opacity-60 grayscale")}>
                   <div className="flex items-center gap-2 text-xs font-semibold text-cyan-600 dark:text-cyan-400 mb-2">
                    ...
                    <div className="text-xs text-muted-foreground mt-1 font-medium">
                      {usage.bonuses?.length ?? 0} {t('common.ok')}
                    </div>
                 </div>
               </div>
            </div>
          ...
          {usage.bonuses && usage.bonuses.length > 0 && (
            <section className="space-y-3">
               <h3 className="font-bold text-sm text-muted-foreground uppercase tracking-wider pl-1">{t('home.bonus_quota')} {t('common.details')}</h3>
                  ...
                  <div key={bonus.code} className="flex items-center justify-between p-4 bg-background border rounded-xl shadow-sm hover:shadow-md transition-shadow">
                    <div className="space-y-1">
                      ...
                      <div className="text-xs text-muted-foreground font-mono">
                        Code: {bonus.code} · {formatDateTime(bonus.expiresAt, i18n.language)} {t('common.expired')}
                      </div>
                    </div>
                    <div className="text-right">
                      ...
                      <div className="text-[10px] text-blue-600 font-medium">
                         {t('home.used')} {((bonus.current / bonus.limit) * 100).toFixed(0)}%
                      </div>
                    </div>
                  </div>
                ))}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
             ...
             <section className="space-y-3">
               <h3 className="flex items-center gap-2 font-bold text-base text-foreground">
                 ...
                 {t('account_detail.basic_info')}
               </h3>
               <div className="bg-muted/30 border rounded-xl p-4 space-y-4">
                 <div className="space-y-1">
                   <label className="text-xs font-medium text-muted-foreground">{t('account_detail.email_address')}</label>
                   <div className="text-sm font-mono break-all select-all">{maskEmail(account.email)}</div>
                 </div>
                 <div className="grid grid-cols-2 gap-4">
                   <div className="space-y-1">
                      <label className="text-xs font-medium text-muted-foreground">{t('account_detail.account_alias')}</label>
                      <div className="text-sm font-medium">{maskNickname(account.nickname) || '-'}</div>
                   </div>
                   <div className="space-y-1">
                      <label className="text-xs font-medium text-muted-foreground">{t('account_detail.identity_provider')}</label>
                      <div className="text-sm font-medium">{account.idp}</div>
                   </div>
                 </div>
                 <div className="space-y-1">
                    <label className="text-xs font-medium text-muted-foreground">{t('account_detail.user_id')}</label>
             ...
             <section className="space-y-3">
               ...
               <div className="bg-muted/30 border rounded-xl p-4 text-sm space-y-3">
                 ...
                 <div className="flex justify-between items-center py-1 border-b border-border/50">
                   <span className="text-muted-foreground text-xs">{t('account_detail.token_expires_at')}</span>
                   <span className="font-medium text-xs">{credentials.expiresAt ? formatDateTime(credentials.expiresAt, i18n.language) : '-'}</span>
                 </div>
                 <div className="flex justify-between items-center py-1 border-b border-border/50">
                   <span className="text-muted-foreground text-xs">{t('account_detail.subscription_type_raw')}</span>
                   <span className="font-mono text-xs" title={subscription.rawType}>{subscription.rawType || '-'}</span>
                 </div>
                 <div className="flex justify-between items-center py-1 border-b border-border/50">
                   <span className="text-muted-foreground text-xs">{t('account_detail.overage_rate')}</span>
                 ...
                 <div className="flex justify-between items-center py-1 border-b border-border/50">
                   <span className="text-muted-foreground text-xs">{t('account_detail.resource_type')}</span>
                   <span className="font-mono text-xs">{usage.resourceDetail?.resourceType || '-'}</span>
                 </div>
                 <div className="flex justify-between items-center py-1">
                   <span className="text-muted-foreground text-xs">{t('home.upgradeable')}</span>
                   <span className={cn("text-xs font-bold", subscription.upgradeCapability === 'UPGRADE_CAPABLE' ? "text-green-600" : "text-muted-foreground")}>
                      {subscription.upgradeCapability === 'UPGRADE_CAPABLE' ? t('common.yes') : t('common.no')}
                   </span>
                 </div>
               </div>
             </section>
